...
mod tests {
	...
	proptest! {
		...
		fn test_nutty_id_chars(uuid in any::<TestUuid>()) {
			...
			assert!(id.as_str().chars().all(|c| valid_chars.contains(c)));
		}
		...
		fn test_nutty_id_ordering(uuid1 in any::<TestUuid>(), uuid2 in any::<TestUuid>()) {
			let id1 = NuttyId::new(uuid1.0);
			let id2 = NuttyId::new(uuid2.0);
